/**
 * Event-Flyer für Gustav (SVG-Data-URL, gleicher Look wie Website).
 */
...
function formatEventFlyerDateLabel(dateInput) {
...
function formatEventFlyerTimeRange(startInput, endInput) {
...
function buildEventFlyerSvg(event = {}) {
...
<svg xmlns="http://www.w3.org/2000/svg" width="${FLYER_W}" height="${FLYER_H}" viewBox="0 0 ${FLYER_W} ${FLYER_H}">
  ...
  <text x="540" y="430" text-anchor="middle" font-family="Georgia, 'Times New Roman', serif" font-size="72" font-weight="700" fill="#3d2817">${title}</text>
  <rect x="120" y="470" width="840" height="6" rx="3" fill="url(#sun)" opacity="0.85"/>
  <text x="540" y="560" text-anchor="middle" font-family="Georgia, serif" font-size="42" font-weight="600" fill="#5a3b24">${dateLabel}</text>
  <text x="540" y="630" text-anchor="middle" font-family="Arial, sans-serif" font-size="36" font-weight="600" fill="#a55c35">🕐 ${timeLabel}</text>
  <text x="540" y="710" text-anchor="middle" font-family="Arial, sans-serif" font-size="30" fill="#5a4634">📍 ${location}</text>
  ${descLines.length ? `<text x="90" y="${descY}" font-family="Arial, sans-serif" font-size="28" fill="#5a4634" font-style="italic">${descSvg}</text>` : ""}
  <rect x="120" y="1120" width="840" height="2" fill="#c67a50" opacity="0.35"/>
  <text x="540" y="1185" text-anchor="middle" font-family="Georgia, serif" font-size="34" font-weight="700" letter-spacing="3" fill="#3d2817">HAUS AM SEE</text>
  <text x="540" y="1235" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" fill="#8a7763">Pilatusstrasse 40 · Pfäffikon ZH</text>
  <text x="540" y="1275" text-anchor="middle" font-family="Arial, sans-serif" font-size="22" fill="#6b9ba8">${escapeXml(WEBSITE_URL.replace("https://", ""))}</text>
</svg>`;
}

function eventFlyerDataUrl(event) {
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(buildEventFlyerSvg(event))}`;
}

function parseFlyerCommand(raw) {
  const s = String(raw || "").trim();
  let m = s.match(/^flyer\s*(?:erstellen|generieren|machen)?\s*(?:für|fuer|von|:)\s*(.+?)\s*$/i);
  if (m) return { title: m[1].trim().replace(/\?+$/, "") };
  if (/^flyer\s*(?:erstellen|generieren|machen)?\s*$/i.test(s)) return { title: null };
  return null;
}

async function attachFlyerToEvent(db, eventId, event) {
  const flyerSrc = eventFlyerDataUrl(event);
  await db.collection("events").doc(eventId).set({ flyerSrc }, { merge: true });
  return flyerSrc;
}
